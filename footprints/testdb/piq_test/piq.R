...
data.path <- "/scratch/data/piq_test"
#-------------------------------------------------------------------------------
# establish database connections:

if(!exists("db.piq"))
    db.piq <- "test_piq_localhost"
...
if(!interactive()){    
    ...
    clusterExport(cl, varlist = c("data.path","db.fimo", "db.piq"),
                  envir = environment())
...
dbConnection <- getDBConnection(db.piq)
